...
## Phase 04 · フロントエンド実装（React + TypeScript）　✅ 完了（2026-07-06）
...
- [x] **TanStack Queryで状態管理** — `QueryClientProvider` 設定済み。一覧取得（`useQuery`）・レビュー（`useMutation` + `invalidateQueries`）を導入済み
