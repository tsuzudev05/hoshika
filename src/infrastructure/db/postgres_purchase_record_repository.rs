...
impl PurchaseRecordRepository for PostgresPurchaseRecordRepository {
    async fn find_by_id(&self, id: Uuid) -> Result<Option<PurchaseRecord>, RepositoryError> {
        let row = sqlx::query(
            "SELECT id, budget_id, wish_item_id, actual_price, memo, purchased_at FROM purchase_records WHERE id = $1",
        )
        .bind(id)
    ...
    async fn save(&self, record: &PurchaseRecord) -> Result<(), RepositoryError> {
        sqlx::query(
            r#"
            INSERT INTO purchase_records (id, budget_id, wish_item_id, actual_price, memo, purchased_at)
            VALUES ($1, $2, $3, $4, $5, $6)
            "#,
        )
        .bind(record.id())
